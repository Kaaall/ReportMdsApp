    ...
    <div class="container my-4">
        ...
        <form method="GET" action="{{ route('surat') }}" class="row g-3 mb-4">
            <div class="col-md-4">
                ...
                <select name="dealer" id="kode_dealer" class="form-select">
                    <option value="">Semua Dealer</option>
                    @foreach ($dealers as $dealer => $namaDealer)
                        <option value="{{ $dealer }}" {{ request('dealer') == $dealer ? 'selected' : '' }}>
                            {{ $namaDealer }}
                        </option>
                    @endforeach
                </select>
        ...
        <form action="{{ route('export.excel') }}" method="POST">
            @csrf
            <input type="hidden" name="dealer" value="{{ $kodeDealer }}">
            <input type="hidden" name="table_type" value="{{ $leasing }}">
